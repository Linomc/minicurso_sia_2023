---
title: "Minicurso_sia"
author: "Lino, Leonardo, Letícia e o orientador Alexandre"
date: "2023-10-08"
output:
...
# Apresentar Markdown para os participantes
* o básico
  1. apresentar o Markdown
  1.1.(Nos Apresentar)
  1.2(Apresentar o projeto do curso)
  1.3()
...
```
### Podemos usar uma biblioteca muito específica:

```{r}
library(skimr)
skim(Turma_estudantes)
```

### O tamanho do Conjunto de Dados?
uma opção é usar o método dim(nome do conjunto de dados)

```{r}
dim(Turma_estudantes)
```
#### O que são essas informações de saída do nosso código? obs. vou repetir [1] para informar as saídas.
* [1] 20 total de **linhas** do conjunto de dados, são as observações, objeto dos cursos de estatística
* [1] 8 total de **colunas** do conjunto de dados, são as variáveis ou atributos, objeto dos cursos de estatística.  

---

OBS. Imaginem, você sendo responsável por informar coisas sobre a turma a partir desse conjunto de dados **Turma_estudantes** será que é possível informar algo ou prof. dessa turma que seja relevante para que o resultados dos alunos que estão no nível básico e intermediário avancem para o nível avançado? Acho que primeiro é necessário conhecer um pouco desses dados, não acham?
...
### Vamos Verificar a associação dessas variáveis, Sexo e Ingles? Realizando o Teste Exato de Fisher

##### obs. o teste a ser escolhido será o teste exato de fisher, é o mais indicado para grupos menores que 50 observações, para conjuntos maiores indicamos o teste qui quadrado 
...
```{r}
bartlett.test(Turma_estudantes$Idade~Turma_estudantes$Ingles) 
```
